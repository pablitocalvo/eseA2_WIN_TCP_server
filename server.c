				...
				if ((n = recv (communication_socket_id, (void*)buffer, sizeof(buffer), 0)) <= 0)
				  {
				   ...
				   break;
					...
					{
					 // ricezione caratteri: costruzione del messaggio
					 for (i=0; i<n; i++)
						if (buffer[i] == '\r' || buffer[i] == '\n')
						  {
						   // messaggio completato: inserimento del terminatore di stringa
						   message[index] = '\0';
						   index = 0;
						   if (strcmp(message, "REQ") == 0)
							 {
							  ...
							  num = htonl(sequence());
							  send (communication_socket_id, (void*)&num, sizeof(unsigned int), 0);
